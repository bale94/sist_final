        ...
        <div class="layout-page">
          ...
          <div class="content-wrapper">
			...
			<div class="container-xxl flex-grow-1 container-p-y">
				<div class="row">
					<h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">인적자원관리 / 업무인원관리 /</span> 소속 프로젝트수정</h4>
					<div class="card">
						<form id="frm01" class="form" name="form" method="post" action="${path}/hresProjList.do">
						<br><br>
							<input type="hidden" name="empno" value="${teammember.empno}">
							<div class="input-group mb-3">
					        	<div class="input-group-prepend">
					            	<span class="text-center input-group-text">사원명</span>
					         	</div>
					         	<input name="name" class="form-control" 
					            	value="${teammember.name }" readonly />   
							</div>  
							<div class="input-group mb-3">
					        	<div class="input-group-prepend">
					         	...
					         	<input name="email" class="form-control" 
					            	value="${teammember.email }" readonly />   
							</div>  
							<div class="input-group mb-3">
					        	<div class="input-group-prepend">
					            	<span class="text-center input-group-text">권한</span>
					         	</div>
					         	<input name="auth" class="form-control" 
					            	value="${teammember.auth }" readonly />   
							</div>  
							<div class="input-group mb-3">
					         	<select aria-label="large select" id="project_no" name="project_no" class="form-select form-select-lg">  
					         		<c:forEach var="mem" items="${teammember.project_no }">	
					         			<option value="${mem.project_no}">${mem.project_no}</option>
					         		</c:forEach>
					         	</select> 
							</div>  
							<div class="text-right">
								<button type="button" onclick="deleteProc()" class="btn btn-success float-end">소속정보삭제</button>
							</div>
						</form> 
					</div>
				</div>
			...
			<script type="text/javascript">
				function deleteProc(){
					if(confirm("소속프로젝트를 삭제하시겠습니까?")){
						//유효성check필요한가
						$("form").attr("action","${path}/hresProjUpt.do");
						$("form").submit();
					}
				}
				var proc = "${proc}"
				if(proc == "upt"){
					if(confirm("수정되었습니다.\n이전화면으로 이동하시겠습니까?")){
						location.href="${path}/hresProjList.do";
					}
				}
			</script>
