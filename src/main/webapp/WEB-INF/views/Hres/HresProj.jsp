...
>
        ...
        <div class="layout-page">
          ...
          <div class="content-wrapper">
			...
			<div class="container-xxl flex-grow-1 container-p-y">
            	<div class="row">
		        	<h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">인적자원관리 /</span> 업무인원관리</h4>
		        	<div class="card">
			        	<form id="schFrm" name="form01" action="" method="post" style="padding-left:50px;">
						    ...
						    <nav class="navbar navbar-expand-sm">
						    	...
						    	<input class="btn btn-primary" type="button" id="schBtn" name="schBtn" value="검색">
						    	<button class="btn btn-outline-primary" type="button" onclick="goReg()">등&nbsp;록</button>
						    </nav>
					        <div class="input-group-prepend">
					            <span class="text-center">총 : ${memberSch.count}명</span>
					        </div>
			        		<div class="float-end">
					        	<div class="input-group-append">
					        		<span class="text-center input-group-text">페이지당 인원</span>
							...
							<table width="850" class="table table-sm table-hover">
								<thead>
									<tr>
										...
										<th scope="col">이메일</th>
										<th scope="col">권한</th>
										<th scope="col">소속프로젝트</th>
									</tr>
								</thead>
								<tbody>
								<c:forEach var="teammember" items="${hresPrjList}">
									<tr height="90px;">
										<td>${teammember.empno}</td>
										<td>${teammember.name}</td>
										<td>${teammember.email}</td>
										<td>${teammember.auth}</td>
										<td>${teammember.project_no}</td>
										<td><button type="button" id="delBtn" class="btn btn-outline-danger btn-sm" 
												 onclick="deleteProc(${teammember.empno})">소속프로젝트 삭제</button></td>
									</tr>
								</c:forEach>
								</tbody>
							</table>
						...
						<ul class="pagination justify-content-end">
							...
							<li class="page-item">
							<a class="page-link" href="javascript:goPage(${memberSch.endBlock+1 })">다음</a></li>
						</ul>
					</div>
				</div>
			</div>
  ...
  <script type="text/javascript">
    	...
    	function deleteProc(empno){
    		if(confirm("소속프로젝트 삭제하시겠습니까?")){
    			$("form").attr("action", "${path}/delProjMem.do?empno="+empno);
    			$("form").submit();
    		}
    	}
    	var proc = "${proc}";
    	...
    	function goReg(){
    		location.href="${path}/hresInsertForm.do";
    	}
	  	$(document).ready(function(){
			$("#schBtn").click(function() {
				$("#schFrm").attr("action", "${path}/hresPrjList.do");
				$("#schFrm").submit();
			});
		})
