...
>
  ...
  <body>
        ...
        <div class="layout-page">
          ...
          <div class="content-wrapper">
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">의사소통관리 /</span> 회의록</h4>
              
              <!-- Minutes List -->
              <h5 class="mt-4">${prj.pname}
              	<span class="badge rounded-pill bg-primary"  style="margin-left: 10px;">
                 진행률 : ${prj.progress}%
                </span>
              </h5>
              <div class="row">
                <div class="col-md mb-4 mb-md-0">
                  <small class="text-light fw-semibold">
                  	<fmt:formatDate pattern="YYYY-MM-dd" value="${prj.prjList.sdate}"/> -  
                    <fmt:formatDate pattern="YYYY-MM-dd" value="${prj.prjList.edate}"/>
                  </small>
                  <c:forEach var="minutes" items="${minutesList}" varStatus="status">
                  <div class="accordion mt-3" id="minutes">
                    <div class="card accordion-item active">
                      <h2 class="accordion-header" id="heading${fn:length(minutesList) -  status.index}">
                        <button
                          type="button"
                          class="accordion-button"
                          data-bs-toggle="collapse"
                          data-bs-target="#accordion${fn:length(minutesList) -  status.index}"
                          aria-expanded="false"
                          aria-controls="accordion${fn:length(minutesList) -  status.index}"
                        >
                          ${fn:length(minutesList) -  status.index}차 회의
                        </button>
                      </h2>
                      <div
                        id="accordion${fn:length(minutesList) -  status.index}"
                        class="accordion-collapse collapse"
                        data-bs-parent="#minutes"
                      >
                        <div class="accordion-body">
			              <input type="hidden" name="project_no" value="${prj.project_no}">
			              <input type="hidden" name="minutes_no" value="${minutes.minutes_no}">
                          <table class="table" style="width: 95%; margin: 0 auto;">
                          	<tr>
                          		<td class="td">회의일시</td>
                          		<td><fmt:formatDate pattern="YYYY-MM-dd HH시 mm분" value="${minutes.mdate}"/></td>
                          		<td class="td">작성일자</td>
                          		<td><fmt:formatDate pattern="YYYY-MM-dd" value="${minutes.wdate}"/></td>
                          	</tr>
                          	<tr>
                          		<td class="td">참석자</td>
                          		<td>${minutes.attendees}</td>
                          		<td class="td">작성자</td>
                          		<td>${minutes.name}</td>
                          	</tr>
                          	<tr>
                          		<td class="td">회의제목</td>
                          		<td colspan="3">${minutes.subject}</td>
                          	</tr>
                          	<tr>
                          		<td class="td">회의내용</td>
                          		<td colspan="3">${minutes.contents}</td>
                          	</tr>
                          	<tr>
                          		<td class="td">결정사항</td>
                          		<td colspan="3">${minutes.decision}</td>
                          	</tr>
                          </table>
                          ...
                          <!-- 
                           ...
                           -->
	                      <div class="btn-group m-4" role="group" style="float: right;">
	                      	<button type="button" class="btn btn-sm btn-outline-primary" 
	                         		onclick="upt(${minutes.minutes_no})">수정</button>
	                        <button type="button" class="btn btn-sm btn-outline-primary"
	                          		onclick="del(${minutes.minutes_no}, ${minutes.project_no})">삭제</button>
	                        <button type="button" class="btn btn-sm btn-outline-primary"
	                          		data-bs-toggle="modal" data-bs-target="#share"
	                          		onclick="goShare('${minutes.mdate}','${minutes.attendees}','${minutes.subject}',
	                          		'${minutes.contents}','${minutes.decision}')">공유</button>
	                    </div>
                        </div>
                      </div>
                    </div>
              ...
              <div class="mt-3" style="float: right;">
 	             <button type="button" class="btn btn-outline-primary" onclick="reg(${prj.project_no})">등록</button>
              </div>
              <!--/ Minutes List -->
	              ...
	              <div class="modal-dialog" role="document">
		              <div class="modal-content">
			              <div class="modal-header">
				              <h3 class="modal-title" id="addMinutes">회의록 공유</h3>
				              <button
				               type="button"
				               class="btn-close"
			              ...
			              <form id="sndMail" action="${path}/sendMinutesMail.do?project_no=${prj.project_no}" method="post">
			              <div class="modal-body">
				              <div class="row">
					              <div class="col mb-3">
						              <input type="hidden" name="mdateS" value=""/>
						              <input type="hidden" name="attendees" value=""/>
			               			  <input type="hidden" name="subject" value=""/>		            			   
						              <input type="hidden" name="contents" value="">
						              <input type="hidden" name="decision" value="">
						              <label for="inputEmail" class="form-label">이메일</label>
						              <input type="email" class="form-control" name="receiver" placeholder="name@example.com">
					              </div>
				              </div>
			              </div>
			              <div class="modal-footer">
				              <button type="button" onclick="share()" class="btn btn-outline-primary" data-bs-dismiss="modal">공유</button>
			              </div>
			              </form>
		              </div>
	              </div>
    ...
    <script type="text/javascript">
		function goShare(mdate, attendees, subject, contents, decision){ 
			$("#sndMail [name=mdateS]").val(mdate)
			$("#sndMail [name=attendees]").val(attendees)
			$("#sndMail [name=subject]").val(subject)
			$("#sndMail [name=contents]").val(contents)
			$("#sndMail [name=decision]").val(decision)
		}		
		function share(){
			if(confirm("공유하시겠습니까?")){
				var rcVal = $("[name=receiver]").val();
				var expText = /^[A-Za-z0-9_\.\-]+@[A-Za-z0-9\-]+\.[A-Za-z0-9\-]+/; 
				if( expText.test(rcVal) == false ){
		...
		if(msg!="") {
			alert(msg);
			location.href="minutesList.do?project_no=${project_no}";
		}
    	function reg(project_no){
    		location.href="regMinutesForm.do?project_no="+project_no;
    	}
    	function upt(minutes_no){
    		location.href="uptMinutesForm.do?minutes_no="+minutes_no;
    	}
    	function del(minutes_no, project_no){
    		if (confirm("삭제하시겠습니까?")) {
				location.href="delMinutes.do?minutes_no="+minutes_no+"&project_no="+project_no;
			}
    	}
