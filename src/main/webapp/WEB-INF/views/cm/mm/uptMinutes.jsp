...
>
  ...
  <body>
        ...
        <div class="layout-page">
          ...
          <div class="content-wrapper">
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">의사소통관리 /</span> 회의록 수정</h4>
                ...
                <div class="col-xl">
                  <div class="card mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                      <h3 class="mb-0">${minutes.project.pname}</h3>
                      <small class="text-muted float-end">
	                  	<fmt:formatDate pattern="YYYY-MM-dd" value="${minutes.project.prjList.sdate}"/> -  
	                    <fmt:formatDate pattern="YYYY-MM-dd" value="${minutes.project.prjList.edate}"/>
                      </small>
                    </div>
                    <div class="card-body">
                      <form action="${path}/uptMinutes.do" method="post">
                      	<input type="hidden" name="minutes_no" value="${minutes.minutes_no}">
                      	<input type="hidden" name="project_no" value="${minutes.project_no}">
                      	<div class="row gy-3 mb-3">
	                        <div class="col-md">
			                	<label class="form-label" for="html5-datetime-local-input">회의일시</label>
			               		<input type="datetime-local" class="form-control" 
			               			   id="html5-datetime-local-input" name="mdateS" 
			               			   value='<fmt:formatDate pattern="yyyy-MM-dd hh:mm" value="${minutes.mdate}"/>'/>
	                        </div>
	                        <div class="col-md">
				            	<label class="form-label " for="html5-datetime-local-input">작성일자</label>
				            	<input type="datetime-local" class="form-control" 
				            	       id="html5-datetime-local-input" name="wdateS" 
				              		   value='<fmt:formatDate pattern="yyyy-MM-dd hh:mm" value="${minutes.wdate}"/>'/>
	                        </div>
                      </div>
                      	<div class="row gy-3 mb-3">
	                        <div class="col-md">
	                        	<label class="form-label" for="basic-default-attendees">참석자</label>
	                        	<input type="text" class="form-control" 
	                        		   id="basic-default-attendees" name="attendees"
	                        	       value="${minutes.attendees}" placeholder="참석자를 입력하세요"/>			                	
	                        </div>
	                        <div class="col-md">
	                        	<label class="form-label" for="basic-default-writer">작성자</label>
	                        	<input type="text" class="form-control" 
	                        	       id="basic-default-writer" name="writer" 
	                        		   value="${minutes.name}" readonly="readonly"/>			                	
	                        </div>
                      </div>
                        <div class="mb-3">
                          <label class="form-label" for="basic-default-subject">회의제목</label>
                          <input type="text" class="form-control" id="basic-default-subject" 
                                 name="subject" value="${minutes.subject}" placeholder="제목을 입력하세요"/>
                        </div>
                        <div class="mb-3">
                          <label class="form-label" for="basic-default-contents">회의내용</label>
                          <textarea class="form-control" 
                                    id="basic-default-contents" 
                          			rows="10" cols="120" 
                          			placeholder="내용을 입력하세요"
                          			name="contents">${minutes.contents}</textarea>
                        </div>
                        <div class="mb-3">
                          <label class="form-label" for="basic-default-decision">결정사항</label>
                          <textarea class="form-control" 
                          			id="basic-default-decision" 
                          			rows="10" cols="120"
                          			placeholder="결정사항을 입력하세요"
                          			name="decision">${minutes.decision}</textarea>
                        </div>
                        <button type="button" class="btn btn-outline-primary" onclick="upt()">수정</button>
                      </form>
                    </div>
                  </div>
                </div>
    ...
    <script type="text/javascript">
    	...
    	var project_no = $("[name=project_no]").val();
    	if(isUpdated=="Y") {
